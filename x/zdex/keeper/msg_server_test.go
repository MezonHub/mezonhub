...
func TestMsgServerCreatePoolSuccess(t *testing.T) {
	...
	swapPool, found := zdexKeeper.GetSwapPool(sdkCtx, lpDenom)
...
func TestMsgServerAddRmProviderShouldWork(t *testing.T) {
	...
	require.NoError(t, err)
	require.True(t, zdexKeeper.HasProvider(sdkCtx, creator2))
	// rm provider
	...
	require.NoError(t, err)
	require.False(t, zdexKeeper.HasProvider(sdkCtx, creator2))
}
...
func TestMsgServerAddRmPoolCreatorShouldWork(t *testing.T) {
	...
	require.NoError(t, err)
	require.True(t, zdexKeeper.HasPoolCreator(sdkCtx, creator2))
	// rm provider
	...
	require.NoError(t, err)
	require.False(t, zdexKeeper.HasPoolCreator(sdkCtx, creator2))
}
...
func TestMsgServerToggleSwitchShouldWork(t *testing.T) {
	...
	require.True(t, zdexKeeper.GetProviderSwitch(sdkCtx))
	...
	require.NoError(t, err)
	require.False(t, zdexKeeper.GetProviderSwitch(sdkCtx))
}
...
func TestMsgServerAddLiquiditySuccess(t *testing.T) {
	...
	_, err = srv.AddProvider(ctx, &msgAddProvider)
	require.NoError(t, err)
	require.True(t, zdexKeeper.HasProvider(sdkCtx, provider))
