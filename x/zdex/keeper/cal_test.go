...
import (
	...
	zdexKeeper "github.com/mezonhub/mezonhub/x/zdex/keeper"
...
func TestCalPoolUnit(t *testing.T) {
	...
	for _, tc := range testcases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			poolUnits, lpunits := zdexKeeper.CalPoolUnit(
				tc.oldPoolUnits,
				tc.nativeAssetBalance,
				tc.externalAssetBalance,
				tc.nativeAssetAmount,
...
func TestCalcSwapResult(t *testing.T) {
	...
	for _, tc := range testcases {
		tc := tc
		t.Run(tc.name, func(t *testing.T) {
			y, fee := zdexKeeper.CalSwapResult(tc.Y, tc.X, tc.x, tc.inputIsBase)
...
func TestCalRemoveAmount(t *testing.T) {
	...
	for _, tc := range testcases {
		...
		t.Run(tc.name, func(t *testing.T) {
			if tc.panicErr != "" {
				require.PanicsWithError(t, tc.panicErr, func() {
					zdexKeeper.CalRemoveAmount(tc.poolUnits, tc.rmUnits, tc.swapUnits, tc.nativeAssetBalance, tc.externalAssetBalance, tc.inputIsBase)
				})
				return
			}

			baseTokenAmount, tokenAMount, swapAmount := zdexKeeper.CalRemoveAmount(tc.poolUnits, tc.rmUnits, tc.swapUnits, tc.nativeAssetBalance, tc.externalAssetBalance, tc.inputIsBase)
