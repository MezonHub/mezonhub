...
func CmdUpdateZValidatorReport() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) (err error) {
			...
			status, exist := types.UpdateZValidatorStatus_value[args[4]]
			if !exist {
			...
			content := types.NewUpdateZValidatorReportProposal(
				clientCtx.GetFromAddress().String(),
				argDenom,
				argPoolAddress,
				&cycle, types.UpdateZValidatorStatus(status))
